...
from option5Class import Convert
...
class buildOption1:#(2-(3**-1.5))
    ...
    def evaluate(tree, opt=0):
        ...
        try:
            if opt == 5:
                convl = Convert(leftTree.getKey())
                leftTree.setKey(convl.convert())
                convr = Convert(rightTree.getKey())
                rightTree.setKey(convr.convert())
        except:
            raise SyntaxError
        if int(mode)==1:
